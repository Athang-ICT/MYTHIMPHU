...
class AclPlugin extends AbstractPlugin 
{
    ...
    public function doAuthorization($e)
    {
		...
		/** Whitelist public routes that don't require ACL check **/
		$publicRoutes = ['auth', 'logout-callback'];
		$publicActions = ['login', 'registration', 'forgotpwd', 'callback', 'logoutCallback'];
		
		if (in_array($routeName, $publicRoutes) || in_array($actionName, $publicActions)) {
			return $acl; // Skip ACL check for public routes
		}
