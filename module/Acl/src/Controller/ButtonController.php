...
class ButtonController extends AbstractActionController
{
	...
	*/
	...
	public function editmenuAction()
	{
		$this->init();
		$id = $this->_id;
		$array_id = !empty($id) ? explode("_", $id) : [];
		$menu_id = isset($array_id[0]) ? $array_id[0] : 0;
	...
	public function editbuttonAction()
	{
		...
		$id = $this->_id;
		$array_id = !empty($id) ? explode("_", $id) : [];
		$button_id = isset($array_id[0]) ? $array_id[0] : 0;
		$page = (sizeof($array_id)>1)?$array_id[1]:'';
		if ($this->getRequest()->isPost()):
			$form = $this->getRequest()->getPost();
			$button = "";
    ...
    {  
		...
		$id = $this->_id;
		$array_id = !empty($id) ? explode("_", $id) : [];
		$process_id = isset($array_id[0]) ? $array_id[0] : 0;
		$page = isset($array_id[1]) ? $array_id[1] : 0;
	...
	{	
		...
		if($process!='0'):
			...
			$permission_array = !empty($process_permissions) ? explode(",", $process_permissions) : [];
			if(isset($permission_array) && is_array($permission_array) && sizeof($permission_array)>0):
    ...
    {  
		...
		$id = $this->_id;
		$array_id = !empty($id) ? explode("_", $id) : [];
		$acl_id = isset($array_id[0]) ? $array_id[0] : 0;
		$page = isset($array_id[1]) ? $array_id[1] : 0;
